...
#             ag_m = torch.eye(N, N).view(1, 1, 1, 1, N, N).repeat(1, 3, 3, 1, 1, 1)
#             ag = ag * (1 - ag_m.float().to(ag.device))
#             ag_m = ag_m * torch.eye(3, 3).view(1, 3, 3, 1, 1, 1)
#             ag = ag + ag_m.float().to(ag.device)
...
class VideoMultiViewModel(nn.Module):
    ...
    def forward(self, pos_2d, bone_angle = None):
        ...
        if self.fuse:
            f, tran, att, f_tmp   = self.fuse_model(f)
            if self.training and self.use_inter_loss:
