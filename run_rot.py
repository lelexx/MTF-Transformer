...
causal_shift = 0
model = VideoMultiViewModel(args, num_view = len(train_cameras) + args.add_view, in_channels = in_channels, channels = args.channels, dropout = dropout, is_train = True, use_inter_loss = use_inter_loss, T = receptive_field)
model_test = VideoMultiViewModel(args, num_view = len(test_cameras) + args.add_view,in_channels = in_channels, channels = args.channels, dropout = dropout, is_train = False, T = receptive_field)
...
if EVAL:
    chk_filename = os.path.join('./checkpoint', args.checkpoint)
...
if True:
    ...
    while epoch < args.epochs:
        ...
        for batch_2d in train_generator.next_epoch():
            ...
            pos_gt = h36_gt
            gt_tran = get_rotation(pos_gt)
            
            gt_tran_2 = gt_tran.permute(0, 1, 2, 3, 5, 4)
            gt_tran = torch.einsum('bqktsm, bkjtsm -> bqjtsm', gt_tran, gt_tran_2)
#             for i in range(5):
#                 for j in range(5):
#                     print(gt_tran[0, :,:,0,i,j])
#                 print('***********')
#             exit()
            ...
            if use_inter_loss:
                loss_total = loss + other_loss + l1loss(tran, gt_tran)
                #print(tran.shape, gt_tran.shape, l1loss(tran, gt_tran))
                #exit()
            else:
                loss_total = loss
            loss_total.backward()
        ...
        with torch.no_grad(): 
            ...
            for num_view in TEST_VIEW:
                for view_list in itertools.combinations(list(range(NUM_VIEW)), num_view):
                    ...
                    for act in vis_actions if EVAL else actions:
                        ...
                        test_generator = ChunkedGenerator(args.batch_size//2, poses_valid_2d, 1,pad=pad, causal_shift=causal_shift, shuffle=False, augment=False,kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right)
                        for batch_2d in test_generator.next_epoch():
                            ...
                            out = model_test(torch.cat((inp, inp_flip), dim = 0))
            ...
            print('avg:{:.2f}'.format(epoch_loss_valid))
            if EVAL:
                exit()
        epoch += 1
        ...
        chk_path = os.path.join('./checkpoint', 'epoch_{}_1.bin'.format(epoch))
...
torch.save({'epoch': best_result_epoch, 'model':best_state_dict}, os.path.join('./checkpoint', 'best.bin'))
